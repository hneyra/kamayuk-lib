 ...
 *
 * Y **no llega del backend**, aunque deberia: ninguno de los cuatro sistemas publica todavia el
 * catalogo de tipos de documento con su longitud. Mientras no lo publique, la comprobacion de
 * longitud la hace la pantalla para que el usuario no envie un DNI de siete digitos y espere a
 * que el servidor se lo rechace; **la que decide sigue siendo la del servidor**, que es la
 * unica que ve el padron entero.
 *
 * <h2>De donde salen los tres</h2>
 *
 * De `const DOCS` del artboard `RentasV6.dc.html:969`, portados a `rentas` en su etapa F-5 y
 * mudados aqui el 2026-09-12 con la creacion de `kamayuk-lib`. **Esa procedencia es la
 * medicion que se hizo y por eso se deja escrita**, pero la comprobacion contra el `.dc.html`
 * NO viaja con el codigo: un artboard es de un sistema y este paquete es de los cuatro. Cada
 * sistema comprueba su propio artboard; lo que este paquete promete es la regla, no el dibujo.
 */
