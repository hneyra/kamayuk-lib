...
| `paquetes/api` — `@kamayuk/api` | **Existe.** `ErrorDeLaApi` y `crearCliente({ prefijo, token })`. **No se extrajo: se diseñó** — medido el 2026-09-12, los tres clientes de `rentas`, `normativa` y `catastro` tenían **tres clases de error incompatibles** y compartían 45 líneas de 135. **Y desde #43 baja documentos**: `Cliente.descargar(ruta, { nombre?, senal? })` devuelve `{ nombre, tipoDeMedio, contenido: Blob }` **sin tocar el DOM** —sus pruebas corren en entorno `node`, donde no hay `document`—, con el mismo prefijo, token y `ErrorDeLaApi` que `solicitar`; un 200 con JSON **lanza** `NoEsUnDocumento` —subclase de `ErrorDeLaApi`, no un `codigo` inventado—, y el nombre sale de `Content-Disposition` antes que del argumento. Entregarlo es otra función, `entregarAlNavegador(documento)`, la única del paquete que toca el DOM: medida en Chromium 151 con la URL revocada al volver del clic, 5 MiB byte a byte. **Y ahora SUBE archivos**: `Cliente.subir<T>(ruta, { archivo, nombre?, campo?, campos?, metodo?, senal?, alAvanzar?, limiteDeBytes?, admite? })` manda un `multipart/form-data` con el mismo prefijo, el mismo token —leído en **cada** llamada, con la misma función que las otras dos— y el mismo `ErrorDeLaApi`. **El `Content-Type` no se fija a mano**: lo pone el navegador con su `boundary`, y lo vigilan dos pruebas, una de ellas sobre **la lista entera** de cabeceras. El transporte es `XMLHttpRequest` y no `fetch`, por una sola razón —el avance de subida, que `fetch` no da sin `duplex: 'half'`, que es solo Chromium y solo HTTP/2, y que mediría lo escrito y no lo recibido—; queda **encerrado en `subir.ts`** y lo vigila `el-xhr-vive-en-un-solo-sitio.test.ts`, que es a la subida lo que `fetch-fuera-del-cliente` es a las lecturas. Se cancela con la misma `senal` y se rechaza con el `reason` que entrega `fetch`. Los tres desenlaces que la pantalla tiene que poder distinguir llegan distinguidos: `ArchivoRechazado` con `motivo` `'demasiado-grande'` o `'tipo-no-admitido'` —local, `estado: 0` y **sin mandar un byte**, o del 413/415 del servidor— y el 422 de la casa como `ErrorDeLaApi`, que es el peldaño «no válido» con su regla dentro. Las clases de error se mudaron a `errores.ts` para romper un ciclo **medido**. **101 pruebas** (eran 52) |
