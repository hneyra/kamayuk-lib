...
| `paquetes/ui` — `@kamayuk/ui` | **Capa 1 hecha (#6).** Los **42** tokens del artboard en un `@theme` de Tailwind v4 —38 colores, 2 radios, 2 sombras—, `Importe`/`Insignia`/`FechaDeCalculo`/`Icono` y los trazos con nombre genérico. **15 pruebas** más las barreras de tipo. Le faltan **los tres temas por dos modos** (capa 2) y los componentes de shadcn, que entran cuando se usen |
