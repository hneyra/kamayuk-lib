...
| `paquetes/verificaciones` — `@kamayuk/verificaciones` | **Existe.** Las **nueve prohibiciones** de ESLint con sus nueve muestras, `sin-suponer-un-sistema`, `sin-nombre-publico-entre-paquetes`, `las-capas-corren`, `los-consumidores-se-miran`, —desde #19— `el-texto-visible-es-dato` mas el arnes de marcas que usan las dos guardas montadas, y —desde #26— `rama-del-consumidor.mjs` con sus muestras y su ensayo contra un remoto de verdad. **78 pruebas.** Le faltan los tokens contra el artboard y el contraste |
